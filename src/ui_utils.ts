...
function formatL10nValue(text: string, args: any[]) {
  if (!args) {
    return text;
  }
 ...
 */
function getOutputScale(ctx: any): any {
 ...
 * @param skipOverflowHiddenElements - Ignore elements that have
 *   the CSS rule `overflow: hidden;` set. The default is false.
 */
function scrollIntoView(element: any, spot: any, skipOverflowHiddenElements = false) {
 ...
 */
function watchScroll(viewAreaElement: Element, callback: Function) {
  let debounceScroll = function(evt: Event) {
    if (rAF) {
      return;
    }
 ...
 */
function binarySearchFirstItem(items: any[], condition: Function) {
  let minIndex = 0;
 ...
 */
function getPageSizeInches(param: { view: any[], userUnit: number, rotate: number }) {
  const { view, userUnit, rotate } = param
 ...
 * @param sortByVisibility - if true, the returned elements are sorted
 *   in descending order of the percent of their padding box that is visible
 * @param horizontal - if true, the elements are assumed to be laid
 *   out horizontally instead of vertically
 * @returns `{ first, last, views: [{ id, x, y, view, percent }] }`
 */
function getVisibleElements(scrollEl: HTMLElement, views: any[], sortByVisibility = false,
                            horizontal = false) {
  ...
  // the padding edge.
  function isElementBottomAfterViewTop(view: any) {
    let element = view.div;
    let elementBottom =
      element.offsetTop + element.clientTop + element.clientHeight;
    return elementBottom > top;
  }
  function isElementRightAfterViewLeft(view: any) {
    let element = view.div;
    let elementRight =
...
function isDataSchema(url: string) {
 ...
 */
function getPDFFileNameFromURL(url: string, defaultFilename = 'document.pdf') {
  ...
  let suggestedFilename = reFilename.exec(splitURI && splitURI[1] || '') ||
                          reFilename.exec(splitURI && splitURI[2] || '') ||
                          reFilename.exec(splitURI && splitURI[3] || '');
...
function normalizeWheelEventDelta(evt: any) {
...
function isValidRotation(angle: any) {
  return Number.isInteger(angle) && angle % 90 === 0;
}

function isPortraitOrientation(size: any) {
...
/**
 * @typedef WaitOnEventOrTimeoutParameters
 * @property target - The event target, can for example be:
 *   `window`, `document`, a DOM element, or an {EventBus} instance.
 * @property name - The name of the event.
 * @property delay - The delay, in milliseconds, after which the
 ...
 *
 * @param
 * @returns A promise that is resolved with a {WaitOnType} value.
 */
function waitOnEventOrTimeout(param: { target: any, name: string, delay: number }) {
  const { target, name, delay = 0 } = param
...
class EventBus {
  ...
  dispatch(eventName: string) {
    ...
    // during dispatch.
    eventListeners.slice(0).forEach(function (listener: Function) {
   ...
   * @private
   */
  _dispatchDOMEvent(eventName: string, args = []) {
...
class ProgressBar {
  ...
  constructor(id: string, param: { height: number, width: number, units: string }) {
    const { height, width, units } = param
    this.visible = true;
 ...
 */
function moveToEndOfArray(arr: any, condition: Function) {
